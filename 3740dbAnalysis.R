...
plotLong$strain <- factor(plotLong$strain, levels = c("WT", "DEL", "COMP", "CROSS"))
...
ggplot(plotLong) +
  ...
  geom_point(aes(x = strain, y = mean,
                 ...
                 size = ifelse(RSD < 0.2,
                               "<20%", ifelse(RSD < 0.5,
                                              "<50%", ">50%")),
                 shape = ifelse(pvalCategoryWTDEL <= 0.001,
                                "<= 0.001", ifelse(pvalCategoryWTDEL <= 0.01,
                                                  "<= 0.01", ifelse(pvalCategoryWTDEL <= 0.05,
                                                                   "<= 0.05", "> 0.05")))),
             stroke = 1.4) +
  theme_bw(base_size = 20) +
  theme(legend.position = "right",
        panel.grid = element_blank()) +
  labs(y = "Percent N-Acetylation",
       x = element_blank(),
       shape = "p-value\n(WT vs DEL)",
       size = "%RSD",
       title = paste0(STRAIN, " (3 Bio Rep x 3 Tech Rep)")) +
...
ggsave(paste0("DBresults/", STRAIN, "_DB_statisticalPlot_ALL_n6.png"),
       width = 12, height = 14)

write.table(plot, paste0("DBresults/", STRAIN, "_pct_Acet.tsv"),
            sep = "\t", row.names = F, quote = F)

sigCutoff <- 0.1
FCCutoff <- 1

plot <- CombinedTandBreps2[CombinedTandBreps2$FC_pctAcet_WTDEL > FCCutoff &
                             !is.na(CombinedTandBreps2$FC_pctAcet_WTDEL) &
                             CombinedTandBreps2$FC_pctAcet_COMPDEL > FCCutoff &
                             !is.na(CombinedTandBreps2$FC_pctAcet_COMPDEL) &
                             CombinedTandBreps2$pvalCategoryWTDEL < sigCutoff &
                             !is.na(CombinedTandBreps2$pvalCategoryWTDEL < sigCutoff),]
...
plotLong$strain <- factor(plotLong$strain, levels = c("WT", "DEL", "COMP"))
...
ggplot(plotLong) +
  ...
  labs(y = "Percent N-Acetylation",
       ...
       shape = "p-value\n(WT vs DEL)",
       size = "%RSD",
       title = paste0(STRAIN, " (2 BR x 3 TR) | FC > ", FCCutoff, " | p-val < ", sigCutoff)) +
  scale_colour_discrete(guide = "none") +
  scale_fill_discrete(guide = "none") +
  scale_size_manual(values = c(5,3.5,2)) +
  scale_shape_manual(values = c(21, 22, 23, 24)) +
...
ggsave(paste0("DBresults/", STRAIN, "_DB_statisticalPlot_FC",FCCutoff, "_pval", sigCutoff, "_n6.png"),
       width = 12, height = 14)
...
ggplot(plotLong) +
  geom_line(aes(x = strain, y = mean,
                group = acetSeq, color = !grepl("^M", plotLong$acetSeq)),
            lwd = 1.2) +
  geom_point(aes(x = strain, y = mean,
                 group = acetSeq, fill = !grepl("^M", plotLong$acetSeq),
                 size = ifelse(RSD < 0.2,
                               "<20%", ifelse(RSD < 0.5,
                                              "<50%", ">50%")),
                 shape = ifelse(pvalCategoryWTDEL <= 0.001,
  ...
  labs(y = "Percent N-Acetylation",
       ...
       shape = "p-value\n(WT vs DEL)",
       size = "%RSD",
       title = paste0(STRAIN, " (2 Bio Rep x 3 Tech Rep)"),
       fill = "Met Cleaved",
       color = "Met Cleaved") +
  ...
  geom_label_repel(data = plotLong[plotLong$strain == "WT",],
                   aes(x = strain, y = mean,
                       label = gene, color = !grepl("^M", plotLong[plotLong$strain == "WT",]$acetSeq)),
                   nudge_x = -1.3, alpha = 0.9,
                   size = 3.5,
                   max.overlaps = 50) +
  coord_cartesian(ylim = c(0,120))
...
ggsave(paste0("DBresults/", STRAIN, "_DB_statisticalPlot_ALL_n6_METCLEAVEDCOLOR.png"),
       width = 12, height = 14)


write.table(CombinedTandBreps2,paste0("DBresults/", STRAIN, "_ALL_protein_summary.tsv"),
           sep = "\t", row.names= F, quote = F)

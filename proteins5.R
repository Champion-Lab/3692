...
png("proteins5/QfeatNormAll.png", units = "px", width = 1200, height = 800)
par(mfrow = c(1, 3))
...
pca_df %>%
  ...
  theme_bw()
#save
ggsave("proteins5/PCA plot for pellet.png", width = 8, height = 8)

#pull out normalized proteins as a dataframe to plot correlation
prot_df <- Qprot[["norm_proteins"]] %>% assay() %>% as.data.frame()
#create correlation matrix
corr_matrixP <- cor(prot_df[, grep("^P", names(prot_df))],
                   method = "pearson",
                   use = "pairwise.complete.obs")
layout(matrix(c(1)), 1)

#save correlation plot
png("proteins5/corrplotPellet.png", width = 1000, height = 1000)
...
#save
ggsave("proteins5/CombinedBayeseanBHcorrectedWTexpressionVSL2F.png", width = 13, height = 18)



#save dataframe as tsv
combined_results$WTexpression <- combined_results$xaxis
write.table(combined_results, "proteins5/AnalyzedResults.tsv",
...
#ggsave("proteins5/CombinedBayeseanBHcorrectedWTexpressionVSL2F_eccs.png", width = 13, height = 18)
ggsave("proteins5/CombinedBayeseanBHcorrectedWTexpressionVSL2F_mbts.png", width = 13, height = 18)
...
ggsave("proteins5/CombinedBayeseanBHcorrectedWTexpressionVSL2F_esx.png", width = 13, height = 18)
